...
Researchers conventionally report $p$-values from two-tailed hypothesis tests. 
...
Thus, our formulation and discussion of these functions is on one-tailed $p$-values based on the null hypothesis of $H_0: \theta \leq 0$ versus the alternative $H_A: $\theta > 0$.
...
<!-- In contrast to step-function models, it does not require the pre-specification of $p$-value thresholds.  -->
<!-- JEP: Took this out because the truncation values _are_ pre-specified thresholds. -->
...
Our simulation results indicate that bootstrap confidence intervals, particularly using the multinomial bootstrap approach with percentile confidence intervals, leads to coverage rates that are close to the nominal level of 0.95 whereas the coverage rates provided by the CRVE method are below nominal. 
Therefore, we strongly recommend using the multinomial bootstrap with the percentile confidence intervals. 
<!-- Cite paper when available -->
...
[^bootstrap-in-practice]: One can interpret these results as a preliminary analysis. For a more refined and reliable analysis, one should refine the analysis by using clustered bootstrap confidence intervals instead of CRVE.
...
<!-- JEP: Revised to remove claim about under-powered meta-analysis. -->
