...
The `metaselection` package implements two different estimation strategies: penalized composite marginal likelihood (PML) and augmented and reweighted Gaussian likelihood (ARGL). 
...
With PML estimation, model parameter estimates are obtained by finding the values that maximize the penalized log-composite-likelihood of the observations, treating each effect size estimate as if it were independent.
...
The selection parameters are estimated using their score equations, just as in the PML approach.
Unlike the PML strategy, the meta-regression coefficients and scale regression coefficients are estimated using the re-weighted log likelihood of the Gaussian evidence-generating process, with weights equal to the inverse probability of selection.
