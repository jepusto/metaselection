...
The `metaselection` package implements two different estimation strategies: composite marginal likelihood and augmented and reweighted Gaussian likelihood.
In the first, model parameter estimates are obtained by taking the values that maximize the log-likelihood of the observations, treating each effect size estimate as if it were independent.
...
Further, the function adds weak prior penalties for the three parameters (for detail, see [`define_priors()`](https://jepusto.github.io/metaselection/reference/define_priors.html)).
By default, the function fits the model using maximum penalized composite marginal likelihood estimation and calculates standard errors and confidence intervals using CRVE.
...
Note that by default the `selection_model()` function uses the weak prior penalties that we described above.
...
The `selection_model()` function also allows fitting models based on beta density selection functions by specifying `selection_type = "beta"`.
The default estimator for the beta function model is composite maximum likelihood; the augmented and reweighted Gaussian likelihood estimator is not yet available.
