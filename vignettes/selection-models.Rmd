...
It is important to note that, although it is possible to specify selection functions in terms of $p$-values from two-tailed hypothesis tests, our focus is on the implementation of these functions using one-tailed $p$-values. Although researchers routinely report two-tailed $p$-values from two-tailed, we focus on one-tailed because prejudice against non-significant results is generally directional, and two-tailed $p$-values do not consider the sign or valence of the effect. Thus, our formulation and discussion of these functions is on one-tailed $p$-values based on the null hypothesis of $H_0: \theta \leq 0$ versus the alternative $H_A: $\theta > 0$.
...
The estimate of the moderator is `r round(mod_3PSM_mod$est$Est[mod_3PSM_mod$est$param=="beta_PRPublicationYes"], 3)`, indicating that studies using within-subjects designs have an average effect that is `r round(mod_3PSM_mod$est["beta_DesignWithin Subjects","Est"], 3)` larger than studies that were not. The moderator accounts for very little of the between-study variability, as show by the `tau2` estimate, which goes down only slightly from `r round(exp(mod_3PSM$est["gamma","Est"]), 3)` to `r round(exp(mod_3PSM_mod$est["gamma","Est"]), 3)` when comparing the three-parameter step function model without and with the moderator.
...
The plot shows that studies with one-sided $p$-values larger than 0.025 are about half as likely ($\lambda_1 = `r round(exp(mod_3PSM_mod$est["zeta1","Est"]), 3)`$) to be published compared to studies with smaller $p$-values.
