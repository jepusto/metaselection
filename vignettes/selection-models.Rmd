...
It is important to note that, although it is possible to implement both selection functions using two-tailed $p$-values, our focus is on the implementation of these functions using one-tailed $p$-values. This is because, although researchers frequently conduct two-tailed tests, prejudice against nonsignificant results is generally directional. Thus, our formulation and discussion of these functions is on one-tailed $p$-values. 
...
One common choice is the three-parameter selection model, which has a single step at $\alpha_1 = .025$ (the default in the package), as depicted in the first figure below. With this choice of threshold, positive effects that are statistically significant at the two-sided level of $p < .05$ have a different probability of selection than effects that are not statistically significant or not in the anticipated direction.
...
The `metaselection` package implements a modification of the beta-density function originally proposed in @Citkowicz2017parsimonious, which involves truncating the selection probabilities at user-specified steps $\alpha_1$ and $\alpha_2$. The truncated beta density is given by
$$
...
Our simulation results indicate that bootstrap confidence intervals, particularly using the multinomial bootstrap approach with percentile confidence intervals, leads to coverage rates that are close to nominal levels whereas the coverage rates provided by the CRVE method are below nominal. We highly recommend using the multinomial bootstrap with the percentile confidence intervals. <!-- Cite paper when available -->
...
As is apparent from the plot, this estimated model indicates that negatively signed effects (i.e., those with a one-sided $p$-value > .50) are even less likely to be observed than effects that are positive but not statistically significant.
...
mod_3PSM_mod <- selection_model(
  ...
  selection_type = "step",
  steps = .025,
  mean_mods = ~PRPublication
)

mod_3PSM_mod
```

The estimate of the moderator is `r round(mod_3PSM_mod$est$Est[mod_3PSM_mod$est$param=="beta_PRPublicationYes"], 3)`, indicating that studies published in a peer reviewed journal have an average effect that is `r round(mod_3PSM_mod$est$Est[mod_3PSM_mod$est$param=="beta_PRPublicationYes"], 3)` larger than studies that were not. This is consistent with selective reporting bias. The moderator also accounts for some of the between-study variability, as show by the `tau2` estimate, which goes down from `r round(mod_3PSM$est$Est[mod_3PSM$est$param=="tau2"], 3)` to `r round(mod_3PSM_mod$est$Est[mod_3PSM_mod$est$param=="tau2"], 3)` when comparing the three-parameter step function model without and with the moderator.

However, the selection parameter `lambda_1` remains virtually unchanged (from `r round(exp(mod_3PSM$est$Est[mod_3PSM$est$param=="zeta1"]), 3)` to `r round(exp(mod_3PSM_mod$est$Est[mod_3PSM_mod$est$param=="zeta1"]), 3)`), which can be seen in the plot below.

```{r 3PSMmod-plot, fig.retina = 2, fig.width = 5, fig.height = 3, out.width = "75%"}
selection_plot(mod_3PSM_mod)
```

The plot shows that studies with $p$-values > 0.025 are about half as likely ($\lambda_1 = `r round(exp(mod_3PSM_mod$est$Est[mod_3PSM_mod$est$param=="zeta1"]), 3)`$) to be published than studies with smaller $p$-values.
