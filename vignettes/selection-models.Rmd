...
Moreover, `RoBMA` package [@RoBMA] implements Bayesian multilevel analysis that accounts for publication bias as described in @bartovs2026robust.
...
One common choice is the three-parameter selection model, which has a single step at $\alpha_1 = 0.025$ (the default in the package), as depicted in the first figure below.
...
```{r, echo = FALSE, fig.retina = 2, fig.width = 5, fig.height = 3, out.width = "75%", fig.cap = "Figure 1. One-step selection model with $\\lambda_1 = 0.4$"}
lambda1 <- 0.4
lambda2 <- 0.2
pvals <- seq(0,1,0.005)
...
```{r, echo = FALSE, fig.retina = 2, fig.width = 5, fig.height = 3, out.width = "75%", fig.cap = "Figure 2. Two-step selection model with $\\lambda_1 = 0.4, \\lambda_2 = 0.2$"}
ggplot(dat, aes(x = pvals)) + 
  scale_y_continuous(limits = c(0,1.1), expand = expansion(0,0)) + 
  scale_x_continuous(breaks = seq(0,1,0.2), expand = expansion(0,0)) + 
...
The `metaselection` package implements a modification of the beta-density function as originally proposed in @Citkowicz2017parsimonious.
The modification involves truncating the selection probabilities at user-specified steps $\alpha_1$ and $\alpha_2$, so that the selection function is given by $$\text{Pr}(O_{ij} = 1 | p_{ij}) = \tilde{p}_{ij}^{(\lambda_1 - 1)} \left(1 - \tilde{p}_{ij}\right)^{(\lambda_2 - 1)},$$ where $\tilde{p}_{ij} = \min\{\max\{p_{ij}, \alpha_1\}, \alpha_2\}$.
...
The first figure below depicts this truncated beta density using the default truncation points and with $\lambda_1 = 0.1, \lambda_2 = 0.9$, which represents very strong selection.
The second figure depicts a truncated beta density with more moderate values of $\lambda_1 = 0.7, \lambda_2 = 1$ and where the second truncation point is set at $\alpha_2 = 0.5$, so that all negative effect size estimates have equal selection probability.
...
The `metaselection` package implements two different estimation strategies: penalized composite marginal likelihood and augmented and reweighted Gaussian likelihood. To stabilize estimation---particularly in smaller meta-analytic datasets or under severe selection conditions where certain regions of the $p$-value distribution may have sparse data---the estimation functions incorporate optional prior penalties on the parameters. The specific penalties applied depend on the choice of selection function (step or beta) and the metric of the data (e.g., standardized mean differences, correlations, raw mean differences, log-odds ratios).

In the first strategy, model parameter estimates are obtained by finding the values that maximize the penalized log-composite-likelihood of the observations, treating each effect size estimate as if it were independent.
...
The selection parameters are estimated using their score equations, just as in the composite marginal likelihood estimator.
...
The plot illustrates how the likelihood of selection differs as a function of the one-sided $p$-value of an effect size estimate.
...
We defined the same set of weak priors as above.
...
The default estimator for the beta-density selection model is penalized composite maximum likelihood; the augmented and reweighted Gaussian likelihood estimator is not yet available for this selection model.
...
The plot shows that effect sizes with smaller $p$-values are more likely to be published than effects with larger $p$-values.
...
We specify `bootstrap = "two-stage"` to run cluster bootstrapping and we specify that we want `"percentile"` bootstrap confidence intervals as the recommended approach.
...
The bootstrapping routine is implemented to work with the `future` package for parallel processing.
