...
step_selection_constraint <- function(
  ...
  S_zeta <- if (is.null(Z0)) scores$S_zeta_ij else cbind(scores$S_zeta0_ij, scores$S_zeta_ij)
...
step_hybrid_jacobian <- function(
  ...
  if (is.null(Z)) {
    if (is.null(Z0)) {
      J_beta_zeta <- -1 * matrix_diag_crossprod(A = X, B = ai * J_beta_zeta_right)
      J_gamma_zeta <- -1 * matrix_diag_crossprod(A = U, B = ai * J_gamma_zeta_right)
      
    } else {
      
      J_beta_zeta_right <- cbind(J_beta_zeta_right[,1] * Z0, J_beta_zeta_right[,-1])
      J_beta_zeta <- -1 * matrix_diag_crossprod(A = X, B = ai * J_beta_zeta_right)
      
      J_gamma_zeta_right <- cbind(J_gamma_zeta_right[,1] * Z0, J_gamma_zeta_right[,-1])
      J_gamma_zeta <- -1 * matrix_diag_crossprod(A = U, B = ai * J_gamma_zeta_right)
    ...
    J_beta_zeta <- -1 * matrix_diag_crossprod(A = X, d = ai * J_beta_zeta_right, B = Z_full)
    J_gamma_zeta <- -1 * matrix_diag_crossprod(A = U, d = ai * J_gamma_zeta_right, B = Z_full)
    
  } else {

    J_beta_zeta_right_h <-  apply(ai * J_beta_zeta_right, 2, identity, simplify = FALSE)
    J_beta_zeta_h <- mapply(matrix_diag_crossprod, d = J_beta_zeta_right_h, B = Z_full, MoreArgs = list(A = X), SIMPLIFY = FALSE)
    J_beta_zeta <- -1 * do.call(cbind, J_beta_zeta_h)
    
    J_gamma_zeta_right_h <- apply(ai * J_gamma_zeta_right, 2, identity, simplify = FALSE)
    J_gamma_zeta_h <- mapply(matrix_diag_crossprod, d = J_gamma_zeta_right_h, B = Z_full, MoreArgs = list(A = U), SIMPLIFY = FALSE)
    J_gamma_zeta <- -1 * do.call(cbind, J_gamma_zeta_h)
    
  }
  
  
  H_zeta <- calculate_H_zeta(
    B_mat = params$B_mat,
    Ai = params$Ai,
    weight_vec = params$weight_vec,
    cats = params$cats,
  ...
  H_zeta_ <- cbind(t(H_zeta$H_beta_zeta), t(H_zeta$H_gamma_zeta), H_zeta$H_zeta_zeta)
                         

  # Put the whole Hessian together
  
  J_matrix <- rbind(
    cbind(J_beta_beta, J_beta_gamma, J_beta_zeta),
    cbind(t(J_beta_gamma), J_gamma_gamma, J_gamma_zeta),
    H_zeta_
  )
...
step_selection_constraint_grad <- function(
  ...
  rbind(H_zeta$H_beta_zeta, H_zeta$H_gamma_zeta, H_zeta$H_zeta_zeta)
}