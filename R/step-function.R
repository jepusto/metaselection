...
step_score <- function(
  ...
  # Derivative of g w/r/t (z_hij * zeta_h)
  # (For VHSM this is the same as g(z_hij * zeta_h) = lambda_h.)
...
step_hessian <- function(
  ...
  # First and second derivative of g w/r/t (z_hij * zeta_h)
  # (For VHSM this is the same as g(z_hij * zeta_h) = lambda_h.)
  ...
  # H_beta_zeta is x_dim * sum(z_dim) or x_dim * (z0_dim + sum(z_dim)) if !is.null(Z0)
  # H_gamma_zeta is u_dim * sum(z_dim) or x_dim * (z0_dim + sum(z_dim)) if !is.null(Z0)
  
  H_zeta <- calculate_H_zeta(B_mat = params$B_mat,
                               Ai = params$Ai,
                               weight_vec = params$weight_vec,
                               cats = params$cats,
                               eta = params$eta,
  ...
  H_matrix <- rbind(
    cbind(H_beta_beta, H_beta_gamma, H_zeta$H_beta_zeta),
    cbind(t(H_beta_gamma), H_gamma_gamma, H_zeta$H_gamma_zeta),
    H_zeta_
  )
