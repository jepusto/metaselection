...
Our metaselection package provides a set of functions that implement these methods.
...
The beta estimate of `r round(mod_3PSM_boot$est$Est[mod_3PSM_boot$est$param=="beta"], 3)`, with a 95% confidence interval `r round(mod_3PSM_boot$est$CI_lo[mod_3PSM_boot$est$param=="beta"], 3)`, `r round(mod_3PSM_boot$est$CI_hi[mod_3PSM_boot$est$param=="beta"], 3)`, represents the overall average effect after accounting for both selection bias and dependent effects. The $p$-value (`r round(mod_3PSM_boot$est$p_value[mod_3PSM_boot$est$param=="beta"], 3)`) indicates that the estimate is not statistically distinct from zero. Tau (`r round( exp(mod_3PSM_boot$est$Est[mod_3PSM_boot$est$param=="gamma"]), 3)`) is the estimated total variance, including both between- and within-study heterogeneity. `lambda1` is the selection parameter. The estimate of `r round(exp(mod_3PSM_boot$est$Est[mod_3PSM_boot$est$param=="zeta1"]), 3)` indicates that effect size estimates with one-sided $p$-values greater than 0.025 are only about half as likely to be reported as estimates that are positive and statistically significant (i.e., estimates with $p < 0.025$).
...
However, because these packages assume effect sizes to be independent, they produce biased estimates for datasets containing multiple effects drawn from the same study.
...
Our package goes beyond these existing related tools by correcting for selective reporting while accommodating meta-analytic datasets that include dependent effect sizes and considering more complex forms of selection.
