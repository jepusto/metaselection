...
beta_score <- function(
  ...
  score_contributions <- get_score_contributions(
    ...
    S_omega_ij = cbind(S_lambda_ij_1, S_lambda_ij_2),
    ai = ai,
...
beta_hessian <- function(
  ...
  H_beta_omega_1_right <-  as.matrix(params$g_dot[1] * (params$dA_dmu * params$dA_dlambda1 / params$Ai^2 - dA_dmu_dlambda1/ params$Ai))
  H_beta_omega_1 <- matrix_diag_crossprod(A = X, B = ai * H_beta_omega_1_right)
  
  H_beta_omega_2_right <-  as.matrix(params$g_dot[2] * (params$dA_dmu * params$dA_dlambda2 / params$Ai^2 - dA_dmu_dlambda2/ params$Ai))
  H_beta_omega_2 <- matrix_diag_crossprod(A = X, B = ai * H_beta_omega_2_right)
  
  H_gamma_omega_1_right <- as.matrix(params$tausq * params$g_dot[1] * (params$dA_deta * params$dA_dlambda1 / params$Ai^2 - dA_deta_dlambda1 / params$Ai))
  H_gamma_omega_1 <- matrix_diag_crossprod(A = U, B = ai * H_gamma_omega_1_right)
  
  H_gamma_omega_2_right <- as.matrix(params$tausq * params$g_dot[2] * (params$dA_deta * params$dA_dlambda2 / params$Ai^2 - dA_deta_dlambda2 / params$Ai))
  H_gamma_omega_2 <- matrix_diag_crossprod(A = U, B = ai * H_gamma_omega_2_right)
  
  
  # H omega omega -----------------------------------------------------------

  H_omega_1_omega_1_right <- 
  ...
  H_omega_1_omega_1 <- sum(ai * H_omega_1_omega_1_right)
  
  H_omega_2_omega_2_right <- 
  ...
  H_omega_2_omega_2 <- sum(ai * H_omega_2_omega_2_right)
  
  H_omega_1_omega_2_right <- 
  ...
  H_omega_1_omega_2 <- sum(ai * H_omega_1_omega_2_right)
  
  # assemble the full Hessian matrix
  
  H_matrix <- rbind(
    cbind(H_beta_beta, H_beta_gamma, H_beta_omega_1, H_beta_omega_2),
    cbind(t(H_beta_gamma), H_gamma_gamma, H_gamma_omega_1, H_gamma_omega_2),
    c(H_beta_omega_1, H_gamma_omega_1, H_omega_1_omega_1, H_omega_1_omega_2),
    c(H_beta_omega_2, H_gamma_omega_2, H_omega_1_omega_2, H_omega_2_omega_2)
  )
