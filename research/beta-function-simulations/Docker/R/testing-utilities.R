...
check_all_derivatives <- function(
  ...
  names(derivs) <- selmod_fit$est$param
  score_diff_over_range <- sapply(derivs, \(x) max(abs(x$d_num - x$score), na.rm = TRUE) / diff(range(x$score, na.rm = TRUE)))
  p_dim <- length(derivs)
  ...
  hess_diff_over_range <- hess_max_diff / hess_range
...
get_selmodel_params <- function(sel_mod, theta = TRUE) {
  ...
  omega <- if (sel_mod$type == "stepfun") log(sel_mod$delta[-1]) else log(sel_mod$delta)
  if (theta) {
    c(beta, gamma, omega)
  } else {
    list(beta = beta, gamma = gamma, omega = omega)
  }
}
...
check_step_score_hessian_bias <- function(
  ...
  beta <- mean_smd
  gamma <- log(tau^2)
  omega <- if (is.list(weights)) as.vector(t(sapply(weights, log))) else log(weights)
...
check_beta_score_hessian_bias <- function(
  ...
  if (fit_mod) {
    ...
    selmod_fit$est$param_val <- c(beta, gamma, omega)
