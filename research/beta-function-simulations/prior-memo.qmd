...
rgnorm <- function(n, mu, phi, L) {
  p <- runif(n, min = -1/2, max = 1/2)
  q <- qgamma(2 * abs(p), shape = 1 / L)
  mu + sign(p) * (q / phi)^(1 / L)
}
```
...
# Priors for $\zeta_1$ and $\zeta_2$ (log of the selection parameters)

For the selection parameters $\zeta_1 = \log(\lambda_1)$ and $\zeta_2 = \log(\lambda_2)$, we consider $L$-norm priors with density
$$
d_\zeta(z) \propto \exp\left(- \phi\left| z - \mu \right|^L \right),
$$
where $\mu$ is the mean (and median and mode), $\phi$ is a precision parameter (i.e., inverse of scale), and $L$ is the norm. We consider two version of the priors, which differ in strength for $\zeta_1$. As a potential default prior for $\zeta_1$, we use $L = 2$, $\mu = \log(0.8)$, and $\phi = \frac{1}{2}$, which implies that $\zeta_1$ follows a normal distribution with mode of $\mu$ and a standard deviation of 1 (i.e., $\lambda_1$ follows a log-normal distribution). As a weaker prior for $\zeta_1$, we use $L = 4$, $\mu = \log(0.5)$, and $\phi = \frac{1}{54}$, which produces a playtkurtic distribution that is very flat over the range $0.2 < \lambda < 1$ but becomes much sharper for $\lambda < 0.05$ or $\lambda > 5$. @fig-zeta-priors depicts the densities of these prior distributions, with the default in red and the weaker prior in purple.
...
#| fig-cap: "Prior distributions on $\\zeta_1$"
#| fig-width: 7
#| fig-height: 4
#| out-width: 100%
...
zeta_weak <- Lp_dist(mu = log(0.5), precision = 1 / 54, L = 4)
...
## Implied beta-density

```{r}
calc_beta <- function(z1, z2, p) {
  tibble(
    p = p, 
    y = beta_fun(delta_1 = exp(z1), delta_2 = exp(z2), renormalize = FALSE)(p)
  )
}
...
sim_settings <- 
  tibble(
    setting = c("extreme", "strong", "moderate","weak", "null"),
    zeta1 = log(c(.01, 0.20, .50, .80, 1)),
    zeta2 = log(c(.90, .90, .90, .90, 1))
  ) %>%
  mutate(
    setting = fct(setting, levels = setting),
    y = map2(zeta1, zeta2, calc_beta, p = p)
  ) %>%
  unnest(y)
...
default_prior_sample <- 
  tibble(
    draw = 1:draws,
    zeta1 = rgnorm(draws, mu = log(0.8), phi = 1 / 2, L = 2),
    zeta2 = rgnorm(draws, mu = log(0.8), phi = 1 / 2, L = 2)
  ) %>%
  mutate(
    y = map2(zeta1, zeta2, calc_beta, p = p)
  ) %>%
...
ggplot(default_prior_sample) + 
  geom_line(aes(p, y, group = draw), alpha = 0.02) + 
  geom_line(data = sim_settings, aes(p, y, color = setting), linewidth = 1.2) + 
  scale_y_continuous(transform = "log", breaks = lambda_bench) + 
  coord_cartesian(ylim = c(0.005,4)) + 
  theme(legend.position = "none") + 
  labs(x = "p", y = "Selection probability")

weak_prior_sample <- 
  tibble(
    draw = 1:draws,
    zeta1 = rgnorm(draws, mu = log(0.5), phi = 1 / 54, L = 4),
    zeta2 = rgnorm(draws, mu = log(1), phi = 2, L = 2)
  ) %>%
  mutate(
    y = map2(zeta1, zeta2, calc_beta, p = p)
  ) %>%
