...
We compared estimates based on the beta density model to estimates generated from three existing methods: (1) a new version of the correlated hierarchical effects model with inverse sampling-covariance weights (CHE-ISCW), which accounts for dependency but not selective reporting [@chen2024adapting]; (2) the PET-PEESE method, which addresses selective reporting and has been adapted to handle dependent data structures [@stanley2014meta]; and (3) a step-function selection model that accounts for both selective reporting and dependent effects [@pustejovsky2025step].
...
<!-- JEP: Cut since it's redundant with comments in the next subsection. -->
<!-- By generating data under the smooth beta-density selection process, we can evaluate the performance of the beta-density selection model under correct specification while simultaneously examining the robustness of the discrete step-function selection model when it is mis-specified. -->
...
We estimated the beta-density selection model using the proposed PML approach with the prior penalties described in `r appendix_prefix` \@ref(prior-specification).
...
First, we estimated a summary meta-analysis model using the CHE-ISCW approach [@chen2024adapting], which accounts for effect size dependence but does not adjust for selective reporting.
...
Second, we estimated a variation of the PET/PEESE model originally proposed by Stanley and Doucouliagos [@stanley2014meta], adapted to handle dependent effect sizes. The PET model regresses effect size estimates on their standard errors, while the PEESE model uses sampling variances instead. 
...
Following established conventions [@stanley2014meta], we used the PET estimate if it was not statistically distinguishable from zero at an $\alpha$-level of 0.10; otherwise, we used the PEESE estimate.

Third, we estimated the step-function selection model using the PML approach. For estimating the step-function models, we used the same prior penalty terms as in prior simulation studies [@pustejovsky2025step]; the prior penalties for $\bs\beta$ and $\tau^2$ are identical to those used for the beta-density model.
...
dat <- tibble(
  ...
  `Full Simulation` = c(
    ...
    "0.0, 0.5",
    "0.40, 0.80",
    "(0.02, 0.90), (0.10, 0.90), (0.20, 0.90), (0.50, 0.90), (1.00, 1.00)",
    "15, 30, 60, 90"
  ),
...
This extrapolation technique was previously evaluated and implemented [@pustejovsky2025step, see Online Appendix C.3 for further details and demonstration of its performance].

## Open Practices

R code and data for replicating the Monte Carlo simulation study are available on the Open Science Framework at https://osf.io/xehgq/overview?view_only=f5d031326aeb40daa7b16b3b9cee5a46. 
