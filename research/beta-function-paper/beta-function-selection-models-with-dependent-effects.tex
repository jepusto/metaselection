...
Recent work addressed this gap by integrating the step-function selection model with robust variance estimation (RVE) or bootstrapping techniques to account for dependent effect sizes\textsuperscript{13}.
...
This model builds on earlier work by Citkowicz and Vevea\textsuperscript{15}, extending it to the context of dependent effect sizes by incorporating RVE and bootstrap methods.
...
Following the approach outlined in previous work on step-function models\textsuperscript{13}, we model the marginal distribution of effect size estimates rather than the joint distribution within studies.
...
Third, it has strong precedents in other areas of statistical analysis, including generalized estimating equations with working independence structures for longitudinal data\textsuperscript{14} and pseudo-likelihood methods for multivariate meta-analysis\textsuperscript{16}.
...
In prior work\textsuperscript{13}, we considered several bootstrap methods designed to preserve the dependence structure across clusters, including the non-parametric clustered bootstrap, two-stage bootstrap\textsuperscript{21,22}, and fractional random weight bootstrap\textsuperscript{23,24}.
...
We first used the \texttt{rma.uni()} function from the \texttt{metafor} package to fit a standard random-effects model based on an independent effects working model with random effects for each effect size\textsuperscript{29}, with standard errors clustered by study.
The overall estimate of the average effect using the unadjusted model was 0.44, cluster-robust 95\% CI {[}0.34, 0.54{]}, significantly different from zero \((p <0.001)\).
The unadjusted estimate of heterogeneity was \(\tau = 0.38\).
...
The unadjusted estimate of heterogeneity was \(\tau = 0.38\), just as in the summary meta-analysis.
...
We compared the beta density model to three existing methods: (1) a new version of the correlated hierarchical effects model with inverse sampling-covariance weights (CHE-ISCW), which accounts for dependency but not selective reporting\textsuperscript{32}; (2) the PET-PEESE method, which addresses selective reporting and has been adapted to handle dependent data structures\textsuperscript{33}; and (3) a step-function selection model that accounts for both selective reporting and dependent effects\textsuperscript{13}.
We evaluated the model estimates based on convergence rates, bias, accuracy, and confidence interval coverage for estimating the average effect size of the unselected distribution.
...
We estimated the beta-density selection model using the PML approach described in Section \ref{estimation-method}.
...
First, we estimated a summary meta-analysis model using the CHE-ISCW approach\textsuperscript{32}, which accounts for effect size dependence but does not adjust for selective reporting.
...
Second, we estimated a variation of the PET/PEESE model originally proposed by Stanley and Doucouliagos\textsuperscript{33}, adapted to handle dependent effect sizes. The PET model regresses effect size estimates on their standard errors, while the PEESE model uses sampling variances instead.
...
Following established conventions\textsuperscript{33}, we used the PET estimate if it was not statistically distinguishable from zero at an \(\alpha\)-level of 0.10; otherwise, we used the PEESE estimate.

Third, we estimated the step-function selection model using the PML approach. As with the beta-density selection model, we used the default penalty structures for the step-function selection model as implemented in the \texttt{metaselection} package\textsuperscript{13}.
...
This extrapolation technique was previously evaluated and implemented\textsuperscript{13} (see Online Appendix C.3 for further details and demonstration of its performance).
