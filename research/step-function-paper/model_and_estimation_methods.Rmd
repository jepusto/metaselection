...
$\Pr\left(O = 1 | p^* \right)$ is proportional to a function $w\left(p^*; \bs\lambda \right)$ that maps $p$-values in the interval $[0,1]$ to strictly positive weights and that involves an unknown $h \times 1$ parameter vector $\bs\lambda$. 

Many different specific selection functions have been proposed in the literature. 
Building on work on random effects models without predictors[@hedges1992modeling], Vevea and Hedges [@vevea1995general] described a random effects meta-regression model where selection probabilities vary depending on a set of pre-specified thresholds for the one-sided $p$-value. 
...
Past developments of selection models have focused on maximum likelihood estimation under the assumption that all effect sizes are mutually independent [@hedges1992modeling; @vevea1995general; @citkowicz2017parsimonious], Bayesian estimation under the assumption of independent effects [@Guan2016bayesian; @Larose1998modeling; @Maier2023robust], or sensitivity analysis methods that treat the selection model as known [@vevea2005publication; @mathur2020sensitivity]. We consider two estimation and inference strategies that build upon and generalize past approaches, including maximum penalized marginal likelihood and an alternative based on re-weighting the Gaussian likelihood of the evidence-generating process.
With both approaches, we incorporate prior penalty terms on all model parameters, which regularize the likelihood and markedly improve the convergence properties of the estimation algorithms.
We also allow for incorporation of weights, which permits efficient calculation for a variety of bootstrapping techniques.
...
For both estimation methods, we incorporate penalty terms that are equivalent to putting independent priors on each element of $\bs\beta$, $\gamma$, and $\bs\zeta$.
The 
...
\end{equation}
We will denote the ARGL parameter estimators as $\bs{\tilde\beta}$, $\tilde\gamma$, and $\bs{\tilde\zeta}$.
