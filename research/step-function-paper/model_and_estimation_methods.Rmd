...
Selection models involve two components [@hedges2005selection]. The first component, which we shall call the evidence-generating process, involves assumptions about the distribution of effect size estimates prior to selective reporting.
...
Doing so permits inference based on the fractional random weight bootstrap [@xu2020applications], also known as the Bayesian bootstrap [@rubin1981bayesian], a technique that can be applied to estimation strategies that admit the use of weights.[^efficiency-weights]
...
l^M\left(\bs\beta, \gamma, \bs\zeta\right) = \sum_{j=1}^J  a_j \sum_{i=1}^{k_j} l^M_{ij}\left(\bs\beta, \gamma, \bs\zeta\right).
\end{equation}
...
\begin{equation}
\sum_{j=1}^J a_j \mat{S}_{j}\left(\bs{\hat\beta}, \hat\gamma, \bs{\hat\zeta}\right) = \mat{0}
\end{equation}
...
\begin{align}
\mat{S}_{\bs\beta j}\left(\bs{\beta}, \gamma, \bs{\zeta}\right) &= \sum_{i=1}^{k_j} \frac{\partial l^M_{ij}\left(\bs\beta, \gamma, \bs\zeta\right)}{\partial \bs\beta} \label{eq:score-M-beta} \\
S_{\gamma j}\left(\bs{\beta}, \gamma, \bs{\zeta}\right) &= \sum_{i=1}^{k_j}\frac{\partial l^M_{ij}\left(\bs\beta, \gamma, \bs\zeta\right)}{\partial \gamma} \label{eq:score-M-gamma} \\
\mat{S}_{\bs\zeta j}\left(\bs{\beta}, \gamma, \bs{\zeta}\right) &= \sum_{i=1}^{k_j} \frac{\partial l^M_{ij}\left(\bs\beta, \gamma, \bs\zeta\right)}{\partial \bs\zeta}. \label{eq:score-M-zeta} 
\end{align}
...
<!-- The assumption that the true parameter values are not at the extremes of their ranges rules out the possibility that the true effect sizes are perfectly explained by the predictors (i.e., a common effect model) and the possibility that effect size estimates with p-values in certain ranges are _never_ published (e.g., only statistically significant effect size estimates are reported).   -->
...
\begin{equation}
\mat{H}(\bs\beta, \gamma, \bs\zeta) = \sum_{j=1}^J a_j \frac{\partial \mat{S}_j(\bs\beta, \gamma, \bs\zeta)}{\partial \left(\bs\beta' \ \gamma \ \bs\zeta'\right)},
\end{equation}
...
\mat{V}^{CML} = \mat{\hat{H}}^{-1}\left(\sum_{j=1}^J a_j^2 \mat{\hat{S}}_j {\mat{\hat{S}}_j}'\right) \mat{\hat{H}}^{-1}.
\end{equation}
...
$$
l^G(\bs\beta, \gamma, \bs\zeta) = \sum_{j=1}^J a_j \sum_{i=1}^{k_j} \frac{1}{w_{ij}} \times l^G_{ij}(\bs\beta, \gamma).
$$
...
\begin{align}
\sum_{j=1}^J a_j \times \mat{S}^G_{\bs\beta j} (\bs\beta, \gamma, \bs\zeta) &= 0 \label{eq:hybrid-score-beta} \\
\sum_{j=1}^J a_j \times \mat{S}^G_{\gamma j} (\bs\beta, \gamma, \bs\zeta) &= 0, \label{eq:hybrid-score-gamma}
\end{align}
where the score contributions for study $j$ are
\begin{align}
\mat{S}^G_{\bs\beta j} (\bs\beta, \gamma, \bs\zeta) &= \sum_{i=1}^{k_j} \mat{x}_{ij}' \frac{y_{ij} - \mat{x}_{ij} \bs\beta}{w_{ij} \left(\exp(\gamma) + \sigma_{ij}^2\right)} \\
\mat{S}^G_{\gamma j} (\bs\beta, \gamma, \bs\zeta) &= \sum_{i=1}^{k_j} \frac{\exp(\gamma)}{2 w_{ij}} \left(\frac{(y_{ij} -\mat{x}_{ij} \bs\beta)^2}{\left(\exp(\gamma) + \sigma_{ij}^2\right)^2} - \frac{1}{\exp(\gamma) + \sigma_{ij}^2}\right).
\end{align}
...
Specifically, we define the ARGL estimators as the values that simultaneously solve Equations \@ref(eq:hybrid-score-beta) and \@ref(eq:hybrid-score-gamma) together with the estimating equation for $\bs\zeta$ given by
\begin{equation}
\label{eq:hybrid-score-zeta}
\mathbf{S}_{\bs\zeta}\left(\bs{\beta}, \gamma, \bs{\zeta}\right) = \sum_{j=1}^J a_j \times \mathbf{S}_{\bs\zeta j}\left(\bs{\beta}, \gamma, \bs{\zeta}\right) = \mathbf{0},
\end{equation}
where $\mathbf{S}_{\bs\zeta j}\left(\bs{\beta}, \gamma, \bs{\zeta}\right)$ is as given in Equation  \@ref(eq:score-M-zeta). 
...
\sum_{j=1}^J a_j \mat{M}_j(\bs\beta, \gamma, \bs\zeta) = \mat{0}.
\end{equation}
...
For the step-function selection process, the score equation with respect to $\bs\zeta$ (Equation \@ref(eq:hybrid-score-zeta)) has an interesting and intuitively intepretable form. 
...
Techniques that do so include the non-parametric clustered bootstrap (also known as the clustered pairs bootstrap) and the fractional random weight bootstrap [@xu2020applications; @rubin1981bayesian]. 
...
A crucial difference between these bootstrap techniques is that the fractional random weight bootstrap puts strictly positive weight on every cluster of observations in every pseudo-sample, whereas the non-parametric bootstrap can assign zero weight to some clusters [@xu2020applications].
...
`r appendix_prefix` \@ref(bootstrap-CIs) provides further details about the bootstrap CI calculations.
