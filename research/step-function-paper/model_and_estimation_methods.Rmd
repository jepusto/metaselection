...
\end{equation}
If $w(y, \sigma; \bs\lambda) = 1$ for all possible $y$, then there is no selective reporting, $A(\mat{x}, \sigma; \bs\beta, \tau^2, \bs\lambda) = 1$, and the density reduces to the unweighted density of the evidence-generating process (i.e., the density of a random-effects meta-regression).
...
Penalized marginal likelihood techniques involve working with the marginal distribution of each observed effect size estimate as if they were all mutually independent [@cox2004note; @lindsay1988composite; @varin2008composite]. 
...
$$
Allowing for prior weights, the inverse selection-weighted Gaussian log-likelihood is
\begin{equation}
...
Specifically, we define the ARGL estimator as the values that maximize \@ref(eq:Gaussian-log-lik) subject to the constraint of the estimating equation for $\bs\zeta$. 

The ARGL estimator can also be defined by a set of estimating equations.
...
For both estimation methods, we aim to specify prior penalties selected to be strong enough to regularize outlying values, but not so strong as to substantially shift the estimator in most datasets.
...
It is strictly weaker than the penalty corresponding to a $N(0,1)$ prior when $|\beta_p| < 2.5$. 
For the log of the heterogeneity parameter, we use 
\begin{equation}
\label{eq:gammaa-prior}
...
\begin{equation}
\label{eq:zeta-prior}
p_{\bs\zeta}(\bs\zeta) = -\frac{1}{54} \sum_{q=1}^z \left(\zeta_q - \log\left(\frac{1}{2}\right)\right)^4
\end{equation}
and depicted in Figure \@ref(fig:prior-penalties)c.
This penalty is centered at selection of $\lambda = \frac{1}{5}$ so that it remains very weak over the range of $\lambda = \frac{1}{20}$ (strong selective reporting) to $\lambda = 1$ (no selective reporting), consistent with the range of selective reporting observed in an empirical review of meta-analyses in psychology and clinical medicine.[@mathur2021estimating]
It more heavily penalizes extremely strong selection levels $(\lambda < \frac{1}{50})$ and selective reporting that systematically _favors_ non-significant findings $(\lambda > 2)$.
