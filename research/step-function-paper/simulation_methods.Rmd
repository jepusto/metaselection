...
of the PML and ARGL estimators for a step-function selection model with a single step, under a wide range of conditions where primary studies contribute multiple, statistically dependent effect size estimates. 
...
of $\tau_B^2$. 
Given $\delta_j$, we generated $k_j^*$ effect size parameters per study, $\bs\delta_j = \left(\delta_{1j},...,\delta_{k_j^* j}\right)'$, from a normal distribution with mean of $\delta_j$ and variance of $\omega^2$.
...
We estimated PML and ARGL estimators and calculated cluster-robust confidence intervals as described in Section \@ref(estimation-methods).[^model-based-SEs] 
...
We compared the performance of the step-function PML and ARGL estimators to that of two alternative methods.
...
This assumption aligns with the default correlation structure used by `robumeta` [@robumeta], the widely used R package for implementing RVE in meta-analysis.
Second, we implemented a variation of the PET/PEESE estimator[@stanley2014meta], adapted to accommodate dependent effect sizes.[^PET-PEESE]
