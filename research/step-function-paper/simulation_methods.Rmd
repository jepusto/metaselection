...
For simplicity in the data-generating process, we assumed a constant correlation between pairs of outcomes within a study but allowed the correlation to vary slightly from study to study.
...
treatment and control group and $\boldsymbol\Psi_j$ is the covariance matrix for outcomes in study $j$, with the diagonal elements equal to 1 and off-diagonal elements equal to $r_j$.[^psi-structure]
...
To maintain computational feasibility, we used $B = 399$ bootstrap replications of each estimator but extrapolated coverage rates for $B = 1999$ replications [@boos2000montecarlo];
`r appendix_prefix` \@ref(bootstrap-extrapolation) provides further details.
