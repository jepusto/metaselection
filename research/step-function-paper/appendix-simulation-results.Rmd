...
```{r winsorization-rates}
#| cache: true
#| fig.width: 9
#| fig.height: 6.5
#| fig.pos: !h
#| fig.env: "sidewaysfigure"
#| fig.cap: "Winsorization rates of CML and ARGL estimators by selection probability, average SMD, and between-study heterogeneity. Points correspond to median winsorization rates; thin lines correspond to range of winsorization rates; thick lines correspond to inter-decile range."

mu_graph_res %>%
  filter(estimator %in% c("CML","ARGL")) %>%
  group_by(mean_smd, tau, weights, estimator) %>%
  summarize(
    min = min(winz_convergence),
    Q10 = quantile(winz_convergence, 0.1),
    median = median(winz_convergence),
    Q90 = quantile(winz_convergence, 0.9),
    max = max(winz_convergence),
    .groups = "drop"
  ) %>%
ggplot() + 
  aes(x = weights, y = median, ymin = min, max = max, color = estimator, fill = estimator) +
