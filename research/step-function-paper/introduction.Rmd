...
This sensitivity analysis provides an estimate of the average effect size after correcting for selective reporting based on a threshold statistical significance level, where the strength of selection is pre-specified by the analyst. It handles effect size dependence using RVE methods [@Hedges2010robust].
...
Another alternative is to use a regression test for small-study effects [as in @egger1997bias], combined with RVE or multilevel meta-analysis to handle dependent effect sizes [@fernandezcastilla2019detecting; @rodgers2021evaluating].
