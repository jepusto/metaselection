...
Recent methodological work on publication bias in univariate meta-analysis has used step-function selection models as one component of an ensemble modeling technique, where it is combined with PET/PEESE regression adjustments and summary (unadjusted) meta-analysis models via Bayesian model averaging.[@Bartos2023robust]
It would be valuable to extend these ensemble techniques to work with dependent effect sizes, but doing so may be hampered by the need of Bayesian methods to work with full likelihoods.
Whether the marginal modeling strategy that we have explored here could be applied with Bayesian model averaging remains an open question, worth further exploration.
...
Based on the simulation results we have presented, we recommend using step function selection models with clustered bootstrap confidence intervals as a primary tool to assess selective reporting bias in syntheses of dependent effect sizes. 
