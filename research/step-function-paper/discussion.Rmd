...
We also studied two estimation methods for the marginal step-function model, penalized marginal likelihood estimation and augmented-and-reweighted Gaussian likelihood estimation, and two inference strategies, based on either cluster-robust sandwich estimators or clustered bootstrap resampling.
...
Compared to the ARGL estimator, the PML estimator of the marginal mean is usually more accurate  and had confidence interval coverage rates closer to nominal levels, although differences are fairly small.
...
Because of this, they do not fully align with contemporary approaches for meta-analysis, which emphasize modeling the hierarchical structure of dependent effect sizes. [@pustejovsky2022preventionscience; @vandennoortgate2013threelevel]
Focusing on marginal models might also be unsatisfying when synthesizing some types of primary studies, such as factorial designs, where the selection process might hinge on the statistical significance of one contrast among the effects (e.g., an interaction effect) rather than the individual effect size estimates.
In further developing this approach, it would be useful to allow for more flexibility by, for instance, letting the heterogeneity parameter vary as a function of covariates as in location-scale models [@viechtbauer2022locationscale] or allowing for the strength of selection to vary based on study characteristics.[@Coburn2015publication]
...
Third, the marginal model does not distinguish between study-level publication bias and effect-level selective outcome reporting.
...
Recent methodological work on publication bias in univariate meta-analysis has combined step-function selection models together with PET/PEESE regression adjustments and summary (unadjusted) meta-analysis models via Bayesian model averaging.[@Bartos2023robust]
It would be valuable to extend such ensemble techniques to work with dependent effect sizes, although doing so may be hampered by the need of Bayesian methods to work with full likelihoods.
...
Second, the simulations examined estimators for the step-function selection model using a selection process that was compatible with the assumed model, in which the probability that an effect size was reported followed a step function in the one-sided _p_-value with a threshold at $\alpha_1 = .025$.
...
Based on the simulation results we have presented, we recommend using step function selection models with two-stage clustered bootstrap confidence intervals as a primary tool to assess selective reporting bias in syntheses of dependent effect sizes. 
