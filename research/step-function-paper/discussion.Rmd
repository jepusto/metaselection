...
Based on the simulation results we have presented, we recommend using step function selection models with clustered bootstrap confidence intervals as a primary tool to assess selective reporting bias in syntheses of dependent effect sizes. As a parametric model built on specific assumptions about the selection process, the marginal step function model provides a robust alternative to more agnostic techniques for identifying small-study effects, such as funnel plots and regression adjustment methods, the bias and accuracy of which are quite variable across data-generating processes.
Our findings suggest that the protection these models provide against systematic bias outweighs the loss in efficiency compared to unadjusted estimates, particularly since the latter can be severely misleading when selective reporting is present.
...
In most practical applications, the bias-adjusted average effect size provides a more credible estimate of the population mean than unadjusted results.
