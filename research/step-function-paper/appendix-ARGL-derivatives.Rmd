...
\begin{equation}
\mat{J}(\bs\beta, \gamma, \bs\zeta) = \sum_{j=1}^J a_j \frac{\partial \mat{M}_j(\bs\beta, \gamma, \bs\zeta)}{\partial (\bs\beta' \gamma \bs\zeta')}.
\end{equation}
...
\mat{V}^{ARGL} = \mat{\tilde{J}}^{-1}\left(\sum_{j=1}^J a_j \mat{\tilde{M}}_j {\mat{\tilde{M}}_j}'\right) \left(\mat{\tilde{J}}^{-1}\right)'.
\end{equation}

The Jacobian of \@ref(eq:hybrid-total-score) is given by
$$
\mat{J} = \sum_{j=1}^J a_j \times \left[\begin{array}{ccc}
...
$$
\begin{aligned}
\mat{J}_j^{\bs\beta\bs\beta'} &= - \sum_{i=1}^{k_j}  \frac{\mat{x}_{ij}' \mat{x}_{ij}}{w_{ij} \eta_{ij}} \\
\mat{J}_j^{\bs\beta \gamma} &= - \sum_{i=1}^{k_j} \mat{x}_{ij}' \tau^2 \left(\frac{y_{ij} - \mu_{ij}}{w_{ij} \eta_{ij}^2} \right) \\
\mat{J}_j^{\bs\beta \zeta_h} &= - \sum_{i=1}^{k_j} \mat{x}_{ij}' \lambda_h \left(\frac{y_{ij} - \mu_{ij}}{w_{ij}^2 \eta_{ij}} \right) \\
\mat{J}_j^{\gamma \bs\beta'} &= \left(\mat{J}_j^{\bs\beta \gamma}\right)' \\
\mat{J}_j^{\gamma \gamma} &= - \sum_{i=1}^{k_j} \frac{\tau^4}{w_{ij}} \left(\frac{2(y_{ij} - \mu_{ij})^2 - \eta_{ij}}{2 \eta_{ij}^3}\right) - \frac{\tau^2}{w_{ij}} \left(\frac{(y_{ij} - \mu_{ij})^2 - \eta}{2 \eta_{ij}^2}\right) \\
\mat{J}_j^{\gamma \zeta_h} &= - \sum_{i=1}^{k_j} \frac{\tau^2 \lambda_h}{w_{ij}^2}  \left(\frac{(y_{ij} - \mu_{ij})^2 - \eta_{ij}}{2\eta_{ij}^2}\right)\\  
