...
# Technical details for the augmented, re-weighted Gaussian likelihood estimator {#ARGL-derivatives}

## Estimating equations for $\bs\beta$ and $\gamma$

We defined the ARGL estimator as the solution to a vector of estimating equations as given in Equation \@ref(eq:hybrid-total-score), where the contribution of study $j$ is
\begin{equation}
...
\end{equation}
as given in Equation \@ref(eq:hybrid-score).
The first two components of $\mat{M}_j(\bs\beta, \gamma, \bs\zeta)$ are given by
\begin{align}
...
\end{align}
The third component is the score equation for $\bs\zeta$ based on the marginal likelihood, as given in Equation \@ref(eq:zeta-score-component).
...
\mat{J}_j^{\bs\beta \zeta_h} &= - \sum_{i=1}^{k_j} a_{ij} \mat{x}_{ij}' \lambda_h \left(\frac{y_{ij} - \mu_{ij}}{w_{ij}^2 \eta_{ij}} \right) I\left(\alpha_h < p_{ij} \leq \alpha_{h+1}\right) \\
...
\mat{J}_j^{\gamma \zeta_h} &= - \sum_{i=1}^{k_j} a_{ij} \frac{\tau^2 \lambda_h}{w_{ij}^2}  \left(\frac{(y_{ij} - \mu_{ij})^2 - \eta_{ij}}{2\eta_{ij}^2}\right) I\left(\alpha_h < p_{ij} \leq \alpha_{h+1}\right) \\  
