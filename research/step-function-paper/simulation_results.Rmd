...
The percentile intervals with two-stage clustered bootstrap re-sampling provided the best coverage levels, achieving or exceeding 90% coverage primarily when between-study heterogeneity is low ($\tau = 0.05$) or when the average effect size is large ($\mu = 0.8$). However, coverage variability increases substantially when heterogeneity is moderate to high ($\tau \geq 0.3$) and the average effect size is small to moderate ($\mu \leq 0.04$). In these conditions, coverage for two-stage clustered bootstrapping generally remained above 80% when $\mu = 0.4$ and stayed above 70% when $\mu \leq 0.02$; coverage for the other methods trailed behind.
