...
[`selection_model()`](reference/selection_model.md), fits step function
and beta density selection models. To handle dependence in the effect
size estimates, the function provides options to use cluster-robust
(sandwich) variance estimation or cluster bootstrapping to assess
...
[`selection_model()`](reference/selection_model.md) function, with
confidence intervals computed using cluster bootstrapping. For further
...
functions to estimate a class of $`p`$-value selection models described
in Vevea and Hedges (1995). However, because these packages assume
effect sizes to be independent, the results they produce will have
incorrect standard errors and misleadingly narrow confidence intervals
for datasets containing multiple effects drawn from the same study. In
